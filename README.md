...
The main window presents three columns: the song library, lyric editor, and a prominent live preview/player. The selected song’s `Title | Singer` title belongs to the editor column. The search field and three toolbar groups are right-aligned together in the window toolbar for song management, lyric import/export, and workspace visibility. The native sidebar control remains in its system-provided leading group, while the editor/player buttons switch between both workspace columns, editor only, and player only.
