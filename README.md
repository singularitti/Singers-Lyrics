...
Choose **New Song from Apple Music** and paste a supported HTTPS Apple Music song link. The app validates the link, looks up its public track metadata, and creates the song only after that lookup succeeds. Title and singer can be changed from the song’s management menu in the library sidebar; replacing the Apple Music link performs a new lookup and updates both values together. Playback first asks Music for the matching title and artist, then falls back to opening the saved link.

The main window presents three columns: the song library, lyric editor, and a prominent live preview/player. A view-only `Title | Singer` header sits in the editor side of the toolbar. The library supports Command-click and Shift-click multi-selection for bulk deletion. When the library is empty, the workspace presents one shared add-song action. The search field and three distinct compact control-group capsules are right-aligned together for song management, lyric import/export, and workspace visibility. The native sidebar control remains in its system-provided leading group, while the editor/player buttons switch between both workspace columns, editor only, and player only. Player mode presents a centered title larger than the lyrics, with the singer before the first lyric; they scroll away with the lyrics but remain outside the timed lyric collection, so line 1 is still synchronization index 0.
