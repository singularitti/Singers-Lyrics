...
The main window presents three columns: the song library, lyric editor, and a prominent live preview/player. The selected song’s larger `Title | Singer` header occupies the editor side of the window toolbar, aligned with the actions on the player side. The search field and three distinct compact control-group capsules are right-aligned together for song management, lyric import/export, and workspace visibility. The native sidebar control remains in its system-provided leading group, while the editor/player buttons switch between both workspace columns, editor only, and player only. Player mode also presents a larger static title and singer above the scrolling lyrics; that header never participates in lyric timing or auto-scrolling.
